...
class HidenLayer:
	def __init__(self, lr, numInput, numOutput, activation = 'tanh', fromModel = None):
		self.lr = lr
...
class MyNN:
	...
	def fit(self, x, index):
		...
		self.layer[2].forward(np.hstack((1,self.layer[1].output)))
		
		# derivation of the output layer loss
		output_err = -(self.label[index] - self.layer[2].output) 
		e2 = output_err**2
		# back propagation
		errs = self.layer[2].backward(output_err, output_layer = 1)
		#print(self.layer[1].w)
		errs = self.layer[1].backward(errs)
		#print(self.layer[0].w)	
		errs = self.layer[0].backward(errs)	
		return e2,self.layer[2].output

	def train(self):
		num_data = self.data.shape[0]
		num_feature = self.data.shape[1] if len(self.data.shape)>1 else self.data.shape[0]
		# defination of the network
		self.layer.append(HidenLayer(0.001,num_feature,num_feature*3))
		self.layer.append(HidenLayer(0.001,self.layer[0].output.shape[0]+1,num_feature*3))
		self.layer.append(HidenLayer(0.001,self.layer[1].output.shape[0]+1,1,activation = 'linear'))
		#print(self.layer[1].w)
	...
	def load_trained_model(self):
		load_file = '.\\trained_model\\trained_model_iteration_2100.mat'
		load_model = sio.loadmat(load_file)
		cnt = 0
		while('layer'+str(cnt) in load_model):
			model_w = load_model['layer'+str(cnt)]
			#act = 'sigmoid' if cnt<2 else 'linear'
			act = 'tanh' if cnt<1 else 'linear'
...
if __name__ == '__main__':
	...
	for it in range(k):	
		...
		if(k!=1):
			...
			# test_data = pd.read_csv('.\\transformed_test.csv', header = None).values
			# test_data = test_data[1:,:]
			# aug_testdata = np.column_stack((np.ones(test_data.shape[0]).transpose(),test_data))
			# output = nn.test(aug_testdata)
			# output[output<0]=0
			# np.savetxt('.\\15352403_zhanggengwei.txt', output, fmt = "%d")
			#np.savetxt('.\\15.csv',output, delimiter = ',', fmt = "%d")
			break	
